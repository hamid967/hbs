...
const db = vi.hoisted(() => ({ getApprovalInbox: vi.fn(), listNotifications: vi.fn() }));
vi.mock("../db", () => db);
...
describe("workboard router", () => { beforeEach(() => { vi.clearAllMocks(); db.listNotifications.mockResolvedValue([]); db.getApprovalInbox.mockResolvedValue([]); }); it("limits a user to their own notifications", async () => { await workboardRouter.createCaller(ctx()).overview(); expect(db.listNotifications).toHaveBeenCalledWith(7, 4); expect(db.getApprovalInbox).not.toHaveBeenCalled(); }); it("uses the direct manager inbox only", async () => { await workboardRouter.createCaller(ctx("manager")).overview(); expect(db.getApprovalInbox).toHaveBeenCalledWith(7, 4, ["manager"]); }); });
