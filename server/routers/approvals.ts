...
import { z } from "zod";
import { decideApprovalTask, getApprovalInbox } from "../db";
...
export const approvalsRouter = router({
  inbox: protectedProcedure.query(({ ctx }) => { const roles = rolesForApproval(ctx.user.role); if (!roles.length) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية صندوق الموافقات" }); return getApprovalInbox(ctx.user.companyId, roles); }),
