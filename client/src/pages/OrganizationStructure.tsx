...
export default function OrganizationStructure() {
  ...
  return <DashboardLayout><div className="mx-auto max-w-7xl" dir="rtl">
    <PageHeader eyebrow="تشغيل متعدد الوحدات" title="البنية التنظيمية" description="كيانات وفروع وفرق ومراكز تكلفة ومواقع عمل مملوكة للشركة الحالية. لا تتغير الأقسام أو ملفات الموظفين القائمة تلقائياً عند إضافة هذه المراجع." actions={canManage ? <Button className="pressable rounded-xl bg-ds-teal-700" onClick={() => setKind("entity")}><Plus className="ml-2 size-4" />إضافة مرجع</Button> : null} />
    ...
    <Dialog open={kind !== null} onOpenChange={open => !open && setKind(null)}><DialogContent dir="rtl"><DialogHeader><DialogTitle>{current?.title}</DialogTitle><DialogDescription>{current?.description}</DialogDescription></DialogHeader><div className="grid gap-4 py-3"><FormField label={current?.name || "الاسم"}><Input value={name} onChange={event => setName(event.target.value)} /></FormField><FormField label={current?.code || "الرمز"}><Input value={code} onChange={event => setCode(event.target.value)} placeholder={kind === "cost-center" ? "مطلوب" : "اختياري"} /></FormField>{["branch", "team", "location"].includes(kind || "") ? <FormField label={kind === "branch" ? "الكيان النظامي" : kind === "team" ? "القسم" : "الفرع"}><Select value={parentId} onValueChange={setParentId}><SelectTrigger><SelectValue placeholder="اختياري" /></SelectTrigger><SelectContent><SelectItem value="none">غير مربوط حالياً</SelectItem>{parentOptions?.map(item => <SelectItem key={item.id} value={String(item.id)}>{item.name}</SelectItem>)}</SelectContent></Select></FormField> : null}{["branch", "location"].includes(kind || "") ? <div className="grid gap-4 sm:grid-cols-2"><FormField label="المدينة"><Input value={city} onChange={event => setCity(event.target.value)} placeholder="اختياري" /></FormField><FormField label="المنطقة"><Input value={region} onChange={event => setRegion(event.target.value)} placeholder="اختياري" /></FormField></div> : null}{["branch", "team"].includes(kind || "") ? <FormField label="المسؤول"><Select value={managerUserId} onValueChange={setManagerUserId}><SelectTrigger><SelectValue placeholder="اختياري" /></SelectTrigger><SelectContent><SelectItem value="none">غير محدد</SelectItem>{employees?.map(employee => <SelectItem key={employee.id} value={String(employee.id)}>{employee.name || "موظف"}</SelectItem>)}</SelectContent></Select></FormField> : null}</div><DialogFooter><Button variant="outline" onClick={() => setKind(null)}>إلغاء</Button><Button onClick={submit} disabled={pending || name.trim().length < 2 || (kind === "cost-center" && code.trim().length < 2)} className="bg-ds-teal-700">حفظ</Button></DialogFooter></DialogContent></Dialog>
  </div></DashboardLayout>;
}
