...
export function LocalLogin() {
  ...
  return <AccessShell icon={<KeyRound />} eyebrow="دخول آمن" title="الدخول بالبريد" detail="استخدم بريد العمل وكلمة المرور التي أنشأتها عبر رابط الدعوة."><form onSubmit={submit} className="space-y-4"><Field label="بريد العمل" value={email} onChange={setEmail} type="email" autoComplete="email" /><Field label="كلمة المرور" value={password} onChange={setPassword} type="password" autoComplete="current-password" /><ErrorText error={login.error?.message} /><Button disabled={login.isPending} className="pressable h-12 w-full rounded-xl bg-[#18b982] font-bold text-[#06201a] hover:bg-[#4bd6aa]">{login.isPending ? "جارٍ التحقق…" : "تسجيل الدخول"}</Button></form><div className="mt-6 border-t border-[#dce4df] pt-5 text-center"><button onClick={() => setLocation("/subscribe")} className="text-sm font-bold text-[#087a5c]">طلب اشتراك جديد</button></div></AccessShell>;
}
...
function Field({ label, value, onChange, type = "text", autoComplete }: { label: string; value: string; onChange: (value: string) => void; type?: string; autoComplete?: string }) { return <label className="block text-right text-sm font-bold text-[#315440]">{label}<Input type={type} value={value} onChange={event => onChange(event.target.value)} autoComplete={autoComplete} required className="mt-2 h-11 rounded-xl border-[#c8d4ce] bg-white" /></label>; }
function ErrorText({ error }: { error?: string }) { return error ? <p role="alert" className="rounded-xl bg-[#fdeceb] p-3 text-xs leading-6 text-[#9f3934]">{error}</p> : null; }
