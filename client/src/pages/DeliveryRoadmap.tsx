  ...
  return (
    <DashboardLayout>
      <div dir="rtl" className="mx-auto max-w-7xl">
        ...
        <section className="mt-8 rounded-3xl border border-[#dbe8de] bg-[#f9fcf9] p-5 md:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="flex items-center gap-2 text-xs font-bold text-[#5d8d70]"><Gauge className="size-4" />برنامج التنفيذ المباشر</p><h2 className="mt-2 text-2xl font-bold text-[#1d4532]">لوحة تقدم المراحل العشر</h2><p className="mt-2 max-w-3xl text-sm leading-7 text-[#758278]">تعرض الحالة الفعلية لكل محور، وتربط إعادة المراجعة بسجل دائم داخل الشركة. لا تبدأ أي عملية رواتب أو وثائق أو تكامل خارجي تلقائياً.</p></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-bold text-[#4b7258]">{directExecutionStages.filter(stage => stage.status === "completed").length} مكتملة · {directExecutionStages.filter(stage => stage.status === "blocked").length} محجوبة</span></div><div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{directExecutionStages.map(stage => { const review = reviewByStage.get(stage.number); const tone = stage.status === "completed" ? "border-[#cfe4d3] bg-white" : stage.status === "partial" ? "border-[#d5dfeb] bg-[#fbfcff]" : "border-[#efdcbf] bg-[#fffaf3]"; const label = stage.status === "completed" ? "مكتملة داخلياً" : stage.status === "partial" ? "تنفيذ جزئي" : review?.status === "retry_requested" ? "إعادة محاولة مسجلة" : review?.status === "dependency_resolved" ? "الاعتمادية محلولة" : review?.status === "review_requested" ? "قيد مراجعة الاعتمادية" : "محجوبة"; return <article key={stage.number} className={`rounded-2xl border p-4 ${tone}`}><div className="flex items-center justify-between gap-2"><span className="flex size-8 items-center justify-center rounded-xl bg-[#edf4ee] text-xs font-bold text-[#397d56]">{stage.number}</span><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${stage.status === "blocked" ? "bg-[#fff1de] text-[#a46723]" : stage.status === "partial" ? "bg-[#e9eef8] text-[#446b98]" : "bg-[#e6f4e8] text-[#347950]"}`}>{label}</span></div><h3 className="mt-3 font-bold text-[#294535]">{stage.title}</h3><p className="mt-2 min-h-12 text-xs leading-5 text-[#718075]">{stage.detail}</p>{stage.dependency && <p className="mt-3 rounded-xl bg-white/80 px-2.5 py-2 text-[10px] leading-5 text-[#8b6937]"><LockKeyhole className="ml-1 inline size-3" />{stage.dependency}</p>}{stage.status === "blocked" && !review && <Button variant="outline" size="sm" onClick={() => setRetryTarget(stage.number)} className="mt-3 h-8 w-full rounded-lg border-[#e6c992] text-xs text-[#9b661d]"><Play className="ml-1 size-3.5" />{stage.retryLabel || "إعادة التحقق"}</Button>}{stage.status === "blocked" && review?.status === "review_requested" && <Button variant="outline" size="sm" onClick={() => setResolveTarget(stage.number)} className="mt-3 h-8 w-full rounded-lg border-[#d0d9e9] text-xs text-[#466994]"><CheckCircle2 className="ml-1 size-3.5" />إقرار حل الاعتمادية</Button>}{stage.status === "blocked" && review?.status === "dependency_resolved" && <Button variant="outline" size="sm" onClick={() => setRetryTarget(stage.number)} className="mt-3 h-8 w-full rounded-lg border-[#cfe0d2] text-xs text-[#397a53]"><Play className="ml-1 size-3.5" />بدء إعادة المحاولة</Button>}</article>; })}</div></section>
