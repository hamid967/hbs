...
export default function EmployeeDirectory() {
  ...
  return <DashboardLayout><div dir="rtl" className="mx-auto max-w-6xl">
    <div className="flex flex-wrap items-end justify-between gap-4">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setDepartmentManagerOpen(true)} className="h-10 rounded-xl border-[#cfe0d3] text-[#286746]">مدير القسم</Button><Button variant="outline" onClick={() => setDesignationOpen(true)} className="h-10 rounded-xl border-[#cfe0d3] text-[#286746]"><BriefcaseBusiness className="ml-2 size-4" />مسمى جديد</Button><Button onClick={() => setDepartmentOpen(true)} className="h-10 rounded-xl bg-[#1f5b45]"><Plus className="ml-2 size-4" />قسم جديد</Button></div>
    </div>
    ...
    <Dialog open={departmentManagerOpen} onOpenChange={setDepartmentManagerOpen}><DialogContent dir="rtl"><DialogHeader><DialogTitle>مدير القسم</DialogTitle><DialogDescription>يُختار حساب مفعّل من الشركة الحالية فقط. هذا التعيين تنظيمي ولا يغير المدير المباشر المخزن في ملف الموظف.</DialogDescription></DialogHeader><div className="grid gap-4 py-3"><Field label="القسم"><Select value={managedDepartmentId} onValueChange={value => { setManagedDepartmentId(value); const department = departments?.find(item => item.id === Number(value)); setDepartmentManagerId(department?.managerUserId ? String(department.managerUserId) : "none"); }}><SelectTrigger><SelectValue placeholder="اختر قسماً" /></SelectTrigger><SelectContent>{departments?.map(item => <SelectItem key={item.id} value={String(item.id)}>{item.name}{item.code ? ` · ${item.code}` : ""}</SelectItem>)}</SelectContent></Select></Field><Field label="مدير القسم"><Select value={departmentManagerId} onValueChange={setDepartmentManagerId} disabled={managedDepartmentId === "none"}><SelectTrigger><SelectValue placeholder="اختر حساباً مفعلاً" /></SelectTrigger><SelectContent><SelectItem value="none">بدون مدير محدد</SelectItem>{employees?.map(item => <SelectItem key={item.id} value={String(item.id)}>{item.name || "موظف"}</SelectItem>)}</SelectContent></Select></Field></div><DialogFooter><Button variant="outline" onClick={() => setDepartmentManagerOpen(false)}>إلغاء</Button><Button onClick={() => saveDepartmentManager.mutate({ departmentId: Number(managedDepartmentId), ...(departmentManagerId !== "none" ? { managerUserId: Number(departmentManagerId) } : {}) })} disabled={saveDepartmentManager.isPending || managedDepartmentId === "none"} className="bg-[#1f5b45]">حفظ المدير</Button></DialogFooter></DialogContent></Dialog>
