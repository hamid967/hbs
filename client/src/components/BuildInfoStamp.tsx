...
export function BuildInfoStamp({ compact = false }: { compact?: boolean }) {
  return (
    <div className={compact ? "flex items-center gap-1.5 text-[10px] text-[#9ab4a5]" : "flex items-center gap-2 text-[11px] text-[#b5cbbd]"} title={`بُني في ${new Date(releaseInfo.builtAt).toLocaleString("ar-SA")}`}>
